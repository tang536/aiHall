...
request.interceptors.request.use(
  config => {
    ...
    // 一致性检查：localStorage 中 token 被外部清除时，同步清除 Pinia store，避免右上角残留账号信息
...
let authFailureHandled = false // 防止多个并发401重复弹窗和跳转
function handleAuthFailure(message) {
  if (authFailureHandled) return
  authFailureHandled = true
  // 3秒后重置标志，允许后续再次触发
  setTimeout(() => { authFailureHandled = false }, 3000)

  const kicked = message && message.includes('其他设备')
  // 同时清除 Pinia store 和 localStorage，避免 isLoggedIn 残留导致路由守卫重定向回首页
  const userStore = useUserStore()
  userStore.logout()
  if (router.currentRoute.value.path !== '/login') {
    if (kicked) {
...
function applyRenewedToken(response) {
  ...
  try {
    const userStore = useUserStore()
    // 同步 Pinia 状态：否则路由守卫读到的还是旧令牌
    if (userStore.token) userStore.token = fresh
  } catch (e) {
    // store 尚未初始化（极早期请求）时，只更新 localStorage 即可
  }
}
...
request.interceptors.response.use(
  response => {
    ...
    if (res.code !== 200) {
      ...
      // 业务错误：保留后端人性化提示文案，并标记 isBiz 供调用方识别
      const err = new Error(res.message || '请求出错')
      err.isBiz = true
