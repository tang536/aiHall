...
**功能新增：**
- 校园地图导航支持路线语音播报：路线规划完成后，点击路线面板的播报按钮，语音朗读出行方式、起终点、全程距离、预计时间及每一步导航指引，播报中按钮闪烁并可随时停止
- AI 对话页支持语音输入（语音转文字）：点击麦克风按钮开始录音，说话内容自动填入输入框，再次点击结束，支持中文连续识别
- AI 对话页支持语音播放（文字转语音）：每条 AI 回复旁有「播放」按钮，点击朗读该条回复，播放中显示「停止」，支持中文语音合成
